...
function sh_psi_fetch(string $url, string $strategy = 'mobile', ?array $settings = null): array
{
    ...
    if ($url === '' || !filter_var($url, FILTER_VALIDATE_URL)) {
        return ['ok' => false, 'demo' => true, 'url' => $url, 'strategy' => $strategy, 'fetched_at' => '', 'scores' => [], 'vitals' => [], 'error' => 'Invalid URL'];
    }
    ...
    if ($key !== '') {
        $apiUrl .= '&key=' . rawurlencode($key);
    }

    $ctx = stream_context_create([
        'http' => [
            'timeout' => 45,
            'header'  => "Accept: application/json\r\nUser-Agent: ShopCMS-PSI/1.0\r\n",
        ],
    ]);
    $raw = @file_get_contents($apiUrl, false, $ctx);
    if ($raw === false || $raw === '') {
        return ['ok' => false, 'demo' => true, 'url' => $url, 'strategy' => $strategy, 'fetched_at' => '', 'scores' => [], 'vitals' => [], 'error' => 'PSI API unreachable'];
    }

    $data = json_decode($raw, true);
    if (!is_array($data)) {
        return ['ok' => false, 'demo' => true, 'url' => $url, 'strategy' => $strategy, 'fetched_at' => '', 'scores' => [], 'vitals' => [], 'error' => 'Invalid PSI response'];
    }
    if (!empty($data['error']['message'])) {
        return ['ok' => false, 'demo' => true, 'url' => $url, 'strategy' => $strategy, 'fetched_at' => '', 'scores' => [], 'vitals' => [], 'error' => (string) $data['error']['message']];
    }
...
function sh_psi_get(string $url, string $strategy = 'mobile', bool $refresh = false, array $langFallback = [], ?array $settings = null): array
{
    ...
    if ($fallbackScores === []) {
        $fallbackScores = [
            ...
            ['label' => 'SEO', 'value' => 100, 'highlight' => true],
        ];
    }
