...
$demoSec = sh_sec_use_demo_snapshot();
$checks = $demoSec ? sh_sec_demo_vulnerability_checks($labels) : sh_sec_vulnerability_checks($labels);
$scoreResult = sh_sec_score($checks);
$result = sh_ai_security_recommendations($settings, $checks, (int) ($scoreResult['score'] ?? 0));
$console = sh_sec_ai_console_lines($checks, $scoreResult, $result);

sh_json_response([
    'ok'              => $result['ok'],
    'demo'            => $demoSec || $result['demo'],
