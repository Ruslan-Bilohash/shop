...
$useDemoSec = sh_sec_use_demo_snapshot();
if ($useDemoSec) {
    $portScan = sh_sec_demo_port_scan($scanHost);
    $server = sh_sec_demo_server_snapshot();
    $vulnChecks = sh_sec_demo_vulnerability_checks($labels);
    $secTrend = sh_sec_demo_trend(14);
} else {
...
$admin_extra_js = [sh_asset('js/admin-security-console.js') . '?v=2'];

require __DIR__ . '/includes/layout.php';
?>

<?php if ($useDemoSec): ?>
<div class="adm-demo-pill adm-sec-demo-pill"><i class="fas fa-shield"></i> <?= htmlspecialchars($sp['demo_snapshot'] ?? 'Demo snapshot — hardened production example (not live server probe)') ?></div>
<?php endif; ?>
<div class="adm-sec-console" id="shSecConsole"
     ...
     data-ai-thinking="<?= htmlspecialchars($sp['ai_thinking'] ?? 'Agent thinking…') ?>"
     data-ai-console-init="<?= htmlspecialchars($sp['ai_console_init'] ?? 'Initializing security agent…') ?>"
     data-ai-console-checks="<?= htmlspecialchars($sp['ai_console_checks'] ?? 'Loading vulnerability checks…') ?>"
     data-ai-console-analyze="<?= htmlspecialchars($sp['ai_console_analyze'] ?? 'Analyzing open issues…') ?>"
     data-ai-console-done="<?= htmlspecialchars($sp['ai_console_done'] ?? 'Scan complete') ?>"
     data-ai-console-error="<?= htmlspecialchars($sp['ai_console_error'] ?? 'Scan failed') ?>"
     data-ai-demo-badge="<?= htmlspecialchars($sp['ai_demo_badge'] ?? 'Demo mode') ?>">
    ...
    <div class="adm-sec-grid adm-sec-grid--extras">
        ...
        <div class="adm-card adm-sec-card">
            ...
            <div class="adm-card-body padded">
                ...
                <div id="shSecAiConsole" class="adm-sec-ai-console" hidden aria-live="polite" role="log"></div>
